...
class TD3:
    ...
    def save(self,env):
        ...
        os.makedirs("src/rl_path_planning/src/rl_path_planning/config/saves/training_weights/"+ env + "/td3_weights", exist_ok=True)
        torch.save(self.PolicyNetwork.state_dict(),"src/rl_path_planning/src/rl_path_planning/config/saves/training_weights/"+env+"/td3_weights/actorWeights.pth")
        torch.save(self.TargetPolicyNetwork.state_dict(),"src/rl_path_planning/src/rl_path_planning/config/saves/training_weights/"+env+"/td3_weights/TargetactorWeights.pth")

    def load(self,env):

        self.PolicyNetwork.load_state_dict(torch.load("src/rl_path_planning/src/rl_path_planning/config/saves/training_weights/"+env+"/td3_weights/actorWeights.pth",map_location=torch.device('cpu')))
        self.TargetPolicyNetwork.load_state_dict(torch.load("src/rl_path_planning/src/rl_path_planning/config/saves/training_weights/"+env+"/td3_weights/TargetactorWeights.pth",map_location=torch.device('cpu')))